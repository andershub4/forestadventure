...
{
    return std::tie(lhs.typeStr_, lhs.x_, lhs.y_, lhs.properties_) ==
           std::tie(rhs.typeStr_, rhs.x_, rhs.y_, rhs.properties_);
}

std::ostream& operator<<(std::ostream& os, const TileMapData::Object& p)
{
    os << "typeStr: " << p.typeStr_ << " x: " << p.x_ << " y: " << p.y_ << " properties: " << p.properties_;
